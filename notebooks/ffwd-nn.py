...
from keras.layers import Input, Dense, Dropout, BatchNormalization, Activation, LeakyReLU, AlphaDropout, Multiply
...
def train_fold(ifold,val_idx,trn_data,val_data,**param):
    ...
    if not "CUDA_VISIBLE_DEVICES" in os.environ:
        os.environ["CUDA_VISIBLE_DEVICES"] = str(ifold)
    
    num_round = 10000
    X_trn, X_nacat_trn, y_trn = trn_data
    X_val, X_nacat_val, y_val = val_data


    ## with tf.device("gpu:%d" % (ifold % 4)):
    L = Input(X_trn.shape[1:])
    Lnacat = Input(X_nacat_val.shape[1:])
    inp = [L, Lnacat]
    Lnacat = Dense(X_trn.shape[-1])(Lnacat)
    Lnacat = Activation("softmax")(Lnacat)
    L = Multiply()([L,Lnacat])
    ...
    for ilayer, layer_size in enumerate(param["layers"]):
        ...
        use_bias = True
        ## if ilayer == 1:
        ##     kernel_reg = regularizers.l1(0.1)
        ##     use_bias = False
    ...
    y_prelim = model.predict( [X_trn,X_nacat_trn] )
    ...
    model.fit( [X_trn,X_nacat_trn], y_trn, epochs=param["epochs"], batch_size=param["batch_size"], validation_data=([X_val,X_nacat_val],y_val), callbacks=callbacks )
...
def train_function(**kwargs):
    ...
    def get_data(trn_idx, val_idx):
        ...
        X_nacat_trn = train.iloc[trn_idx][nacat].values
        y_trn = target.iloc[trn_idx]
        X_val = train.iloc[val_idx][features].values
        X_nacat_val = train.iloc[val_idx][nacat].values
        X_val[ np.isnan(X_val) | np.isinf(X_val.astype(np.float)) ] = 0.
        y_val = target.iloc[val_idx]

        return [(X_trn,X_nacat_trn,y_trn),(X_val,X_nacat_val,y_val)]

    results = Parallel(n_jobs=5,verbose=10)(delayed(train_fold)(ifold,val_idx,*get_data(trn_idx, val_idx),**param)
    ...
    for val_idx, model_name in results:        
        ...
        X_val[ np.isnan(X_val) | np.isinf(X_val.astype(np.float)) ] = 0.
        X_nacat_val = train.iloc[val_idx][nacat].values
        oof[val_idx] = model.predict([X_val,X_nacat_val]).ravel()
...
categorical_features = [c for c in train.columns if 'feature_' in c]
...
for feat in categorical_features:
    ...
    enc.fit(train[[feat]])
    ## print(type(enc.transform(train[[feat]])))
...
opt_pars = {"depth": 3.445280813592266, "dropout": 0.20587926030420933, "lr": -2.5269383900889872, "width": 4.757298632110292, "width_drop_lenght": 1.346436156950145, "width_drop_start": 5.910975003691972}

default_param.update(opt_pars)

X_test = test[features].values
X_test[ np.isnan(X_test) | np.isinf(X_test.astype(np.float)) ] = 0.
X_nacat_test = test[nacat].values

X_test = [X_test,X_nacat_test]
...
## sub_df.to_csv("submit-nn-baseline-l10p5.csv", index=False)
sub_df.to_csv("submit-nn-opt-fix.csv", index=False)

valid_df = pd.DataFrame({"card_id":train["card_id"].values})
valid_df["target"] = oof
valid_df.to_csv("valid-nn-opt-fix.csv", index=False)
## valid_df.to_csv("valid-nn-baseline-l10p5.csv", index=False)
...
### 
### optimizer.maximize(init_points=9,
###                    n_iter=50,kappa=5.)
### 
### 
